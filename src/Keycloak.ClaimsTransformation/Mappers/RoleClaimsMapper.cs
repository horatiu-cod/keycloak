...
namespace Keycloak.ClaimsTransformation.Mappers;

internal static class RoleClaimsMapper
{
    ...
    private static List<string> GetRealmRoles(ClaimsIdentity identity)
    {
        ...
        if (identity.TryGetClaim(RealmResourceAdapter, out var resourceAccessClaim) && resourceAccessClaim is not null)
        {
            var resource = JsonDocument.Parse(resourceAccessClaim.Value);
